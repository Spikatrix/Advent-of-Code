...
let syncFlashStep: number = -1;
for (let step = 1; syncFlashStep === -1; step++) {
	...
	const backupFlashCount: number = flashCount;
	...
	if (flashCount - backupFlashCount === octopusEnergy.length * octopusEnergy[0].length) {
		syncFlashStep = step;
	}
}
